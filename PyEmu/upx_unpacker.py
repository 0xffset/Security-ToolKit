from ctypes import *
from PyEmu import PEPyEmy
'''
HMODULE WINAPI LoadLibrary(
	___in LPCTSTR lpFileName
);
'''
def loadlibrary(name, address):
	# Retrive the DLL name
	dllname = emu.get_memory_string(emu.get_memoy(emu.get_memory_register("ESP") +4))
	...
	return_address = emu.get_memoy(emu.get_register("ESP"))
...
def virtualprotect(name, address):
	...
	# Reset the stack and return from out handler
	return_address = emu.get_memoy(emu.get_register("ESP"))
